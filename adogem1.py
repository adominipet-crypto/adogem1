...
def main():
    ...
    for code, s in sorted(selected_stocks.items()):
        prefix = '★PPP ' if '★PPP ' in s['ppp_label'] else ''
        final_list.append(f"  {prefix}■ {code} | {s['price']}円 ({s['date'][5:]})")
    ...
    stage_count_map = {
        "stage6": stage_survivors['stage6'], "stage7": stage_survivors['stage7'],
        "stage8": stage_survivors['stage8'], "stage9": stage_survivors['stage9'],
