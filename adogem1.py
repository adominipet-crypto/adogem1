...
def fetch_global_latest_date():
    ...
    try:
        ...
        print(f"【DEBUG】日経平均から取得した基準日 (GLOBAL_LATEST_DATE): {GLOBAL_LATEST_DATE}")
        ...
        print(f"【DEBUG】日経平均取得エラーのため代替日を使用: {GLOBAL_LATEST_DATE} (理由: {e})")
...
def get_stock_data_fallback(symbol, force_check_date=True):
    ...
    try:
        ...
        # HTTPエラー（アクセス制限など）の検知
        if res.status_code != 200:
            if res.status_code == 429:
                print(f"【⚠️警告】Yahoo Financeからアクセス制限(429 Too Many Requests)を受けています。")
            return None
        ...
        # 日付チェックで落とされているかどうかの検証ログ
        if force_check_date and GLOBAL_LATEST_DATE and df.index[-1].date() != GLOBAL_LATEST_DATE:
            if debug_mismatch_printed < 5:  # ログが埋まらないよう最初の5件だけ出力
                print(f"【DEBUG】銘柄 {symbol} が日付不一致でスキップされました: 銘柄最新日={df.index[-1].date()} vs 基準日={GLOBAL_LATEST_DATE}")
                debug_mismatch_printed += 1
            return None
            
        return df
    except Exception as e:
        if symbol in ["1301", "7203"]: # 代表的な銘柄でエラーが出ていれば出力
            print(f"【DEBUG】銘柄 {symbol} のデータ処理中に例外発生: {e}")
        return None
...
def record_to_spreadsheet():
    ...
    except Exception as e: print(f"シート1への追記エラー: {e}")
    ...
    try:
        if selected_stocks:
            ...
            for code, r in selected_stocks.items():
                row1 = [r["date"], code, "", r["price"], "翌日終値"] + [f"{d}営業日" for d in range(3, 16)]
                padding = [""] * (17 - len(row1)) if len(row1) < 17 else []
                row1 = row1 + padding + [r["date"], code, r["price"]]
                
                row2 = ["通過条件ステージ", "12. 天井圏維持", "", ""] + ["判定"] * 14
                padding2 = [""] * (17 - len(row2)) if len(row2) < 17 else []
                row2 = row2 + padding2 + ["通過条件ステージ", "12. 天井圏維持", ""]
                
                row3 = ["PPP", r["ppp_label"].strip() or "通常", "", ""] + ["前日比(%)"] * 14
                padding3 = [""] * (17 - len(row3)) if len(row3) < 17 else []
                row3 = row3 + padding3 + ["PPP", r["ppp_label"].strip() or "通常", ""]
            ...
            if new_rows_s2:
                ...
                print(f"シート2に完全合格銘柄を {len(selected_stocks)} 件正しく追記しました。")
    except Exception as e: print(f"シート2への追記エラー: {e}")

# --- メイン処理 ---
def main():
    fetch_global_latest_date()
    
    # 1. 過去データの自動答え合わせ・およびシート2の大修復を実行
    update_yesterday_results()
    update_sheet2_results()
    
    # 2. 当日の全銘柄スクリーニング
    start_r, end_r = (int(sys.argv[1]), int(sys.argv[2])) if len(sys.argv) > 2 else (1300, 10001)
    print(f"【処理開始】対象範囲: {start_r} 〜 {end_r}")
    ...
    for s in [str(i) for i in range(start_r, end_r)]: 
        ...
        # 1000件ごとにログを出して生存を確認する
        if scan_count % 1000 == 0:
            print(f"【進捗】現在 {scan_count} 件の銘柄をスキャン完了...")
    ...
    nikkei_block = "【日経平均の判定】\n" + ("\n".join(nikkei_report_lines) if nikkei_report_lines else "  データなし")
    judgement_summary_block = f"【確定の判定結果】\n  {final_judgement_summary if final_judgement_summary else '判定データなし'}"
