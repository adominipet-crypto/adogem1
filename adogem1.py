...
def get_stock_data_fallback(symbol, force_check_date=True):
    try:
        ...
        if res.status_code != 200:
            if int(symbol) % 100 == 0:
                print(f"[DEBUG] 銘柄 {symbol} 取得失敗。ステータスコード: {res.status_code}")
            return None
        ...
        if int(symbol) % 100 == 0:
            print(f"[DEBUG] 銘柄 {symbol} 例外発生: {e}")
        return None
...
def get_nikkei_evaluation_line():
    try:
        ...
        # 最新データ日（GLOBAL_LATEST_DATE）までのデータに絞り込む
...
def update_yesterday_results():
    ...
    try:
        ...
        # 7/1時点の最新データで答え合わせをするため、選定された日（前営業日：6/30）の日付をターゲットにする
        if GLOBAL_LATEST_DATE:
        ...
        for i, row in enumerate(all_records):
            ...
            try: 
                ...
                # スプレッドシート内の選定日付が「最新データ日（7/1）の前営業日（6/30）」と一致する場合のみ答え合わせを実行
